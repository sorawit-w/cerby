...
| Destructive git (`push --force`, protected-branch push, `reset --hard`, `clean -f`, `branch -D`, wholesale discard) + commit while on a protected branch | `protect-git.sh` (PreToolUse Bash) hard-block | `[enforced-when-installed]` | Regex-matched on the command string; exotic shell obfuscation could evade — `protect-git.test.sh` covers the common forms. The commit gate parses the git subcommand (so `git log --grep=commit` is not a commit), reads the **target** repo's live branch (resolving a single `git -C <path>`), and is *escapable* via an inline `CODING_RULES_ALLOW_PROTECTED_COMMIT=1` prefix (workflow guard, not data loss) — the destructive blocks are not. **Residual:** a single PreToolUse pass can't fully model runtime git — global options are matched by *shape* (so `-P`/`--config-env=…` and future globals are handled), but a *separate-arg* long global not in the known set (`--foo /path commit`), multiple cumulative `-C` with relative paths, or a quoted path containing spaces can still evade or mis-resolve. A git `pre-commit` hook (runs in-repo at commit time) would be bulletproof but is a different mechanism; not adopted here to keep one install model |
